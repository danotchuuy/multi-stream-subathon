// Service definition for YouTube's liveChatMessages.streamList gRPC
// method, transcribed from:
...
type LiveChatMessageSnippet struct {
	...
	// The date and time when the message was originally published. The
	...
	// not present the message is silent, at the moment only messages of
...
type LiveChatMessageSnippet_SuperStickerDetails struct {
	// Details about the Super Sticker event, this is only set if the
	// type is 'superStickerEvent'.
...
type LiveChatMessageSnippet_NewSponsorDetails struct {
	// Details about the New Member Announcement event, this is only set
	// if the type is 'newSponsorEvent'. Note that "member" is the new
	// term for "sponsor".
...
type LiveChatMessageSnippet_MembershipGiftingDetails struct {
	// Details about the Membership Gifting event, this is only set if
	// the type is 'membershipGiftingEvent'.
...
type LiveChatNewSponsorDetails struct {
	...
	// names are defined by the YouTube channel offering the Membership.
	//
...
type LiveChatMemberMilestoneChatDetails struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The name of the Level at which the viewer is a member. The Level
	// names are defined by the YouTube channel offering the Membership.
	//
	// In some situations this field isn't filled.
	MemberLevelName *string `protobuf:"bytes,1,opt,name=member_level_name,json=memberLevelName" json:"member_level_name,omitempty"`
...
type LiveChatGiftMembershipReceivedDetails struct {
	...
	// the snippet.membershipGiftingDetails.giftMembershipsLevelName of the
	// associated membership gifting message. The Level names are defined
	// by the YouTube channel offering the Membership.
	//
	// In some situations this field isn't filled.
	MemberLevelName *string `protobuf:"bytes,1,opt,name=member_level_name,json=memberLevelName" json:"member_level_name,omitempty"`
...
type SuperChatEventSnippet struct {
	...
	// The purchase amount, in micros of the purchase currency. For
	// example, 1 is represented as 1000000.
...
// Paging details for lists of resources, including total number of
...
const file_stream_list_proto_rawDesc = "" +
	...
	"\n" +
	"StreamList\x12*.youtube.api.v3.LiveChatMessageListRequest\x1a+.youtube.api.v3.LiveChatMessageListResponse\"\x000\x01BBZ@github.com/danotchuuy/multi-stream-moderation/internal/youtubepb"
